    ...
    public class TicketController : Controller
    {
        ...
        public ActionResult AddTicket(FormCollection formCollection, HttpPostedFileBase attachment)
        {
            ...
            if (quickTicketId == 0)
            {
                ...
                var uers = users.GetUsers(out totalCount, roleId: 5);
            ...
            else
            {
                ...
                int totalCount = 0;
                var uers = users.GetUsers(out totalCount, roleId: rolId);
                ticket.TicketAssignedToStaff = new List<AssignTicket>();
                foreach (var user in uers)
                {
                    ticket.TicketAssignedToStaff.Add(new AssignTicket() { UserId = user.Id });
            ...
            ticket.TicketNo = userSession.Id + DateTime.Now.ToString("yyyyMMdd") + rnd.Next();
        ...
        public List<User> GetAllAssignedUsers()
        {
            ...
            var users = userService.GetUsers(out totalCount);//.Where(x => x.IsCustomer == false && x.IsAdmin == false).ToList();
            return users;
        }
